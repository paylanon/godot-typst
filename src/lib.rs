...
use godot::prelude::*;
use godot::engine::{Sprite2D, ISprite2D, Image, ImageTexture, TextureRect, ITextureRect};
...
#[godot_api]
impl ISprite2D for TypstSprite {
...
pub struct TypstTextureRect {
    #[base]
    pub node: Base<TextureRect>,
    #[export(multiline)]
    pub typst_expression: GString,
    pub stored_expr: GString,
    pub shared_queue: Arc<Mutex<Vec<Vec<u8>>>>,
...
#[godot_api]
impl ITextureRect for TypstTextureRect {
    fn init(node: Base<TextureRect>) -> Self {
        TypstTextureRect {
            node,
            typst_expression: String::new().into(),
            stored_expr: String::new().into(),
